        ...
        <ol class="breadcrumb">
            ...
            <c:forEach items="${categoryList}" var="category">
                <c:if test="${cid==category.cid}">
                    <li>
                        <a href="productList?currentPage=1&cid=${pageContext.request.getParameter("cid")}">${category.cname}</a>
                    </li>
                </c:if>
            </c:forEach>
...
<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
    <ul class="pagination" style="text-align: center; margin-top: 10px;">
        ...
        <c:if test="${currentPage!=1}">
            <li><a href="productList?currentPage=${currentPage-1}&cid=${pageContext.request.getParameter("cid")}"
        ...
        <c:forEach begin="1" end="${totalPage}" var="pageNum">
            <c:if test="${currentPage==pageNum}">
                <li class="active"><a
                        href="productList?currentPage=${pageNum}&cid=${pageContext.request.getParameter("cid")}">${pageNum }</a>
                </li>
            </c:if>
            <c:if test="${currentPage!=pageNum }">
                <li>
                    <a href="productList?currentPage=${pageNum}&cid=${pageContext.request.getParameter("cid")}">${pageNum}</a>
                </li>
            </c:if>
        </c:forEach>
