...
<head>
    ...
    <title>会员登录</title>
<%--    <title>${searchName}-商品搜索-惠多多</title>--%>
...
<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
    <ul class="pagination" style="text-align: center; margin-top: 10px;">
        ...
        <c:if test="${currentPage!=1}">
            <li><a href="SearchProductServlet?currentPage=${currentPage-1}&search=${pageContext.request.getParameter("search")}" aria-label="Previous"><span
                    aria-hidden="true">&laquo;</span></a></li>
        </c:if>
        ...
        <c:forEach begin="1" end="${totalPage}" var="pageNum">
            <c:if test="${currentPage==pageNum}">
                <li class="active"><a
                        href="SearchProductServlet?currentPage=${pageNum}&search=${pageContext.request.getParameter("search")}">${pageNum }</a></li>
            </c:if>
            <c:if test="${currentPage!=pageNum }">
                <li><a href="SearchProductServlet?currentPage=${pageNum}&search=${pageContext.request.getParameter("search")}">${pageNum}</a></li>
            </c:if>
        </c:forEach>

        <c:if test="${currentPage!=totalPage}">
            <li><a href="SearchProductServlet?currentPage=${currentPage+1}&search=${pageContext.request.getParameter("search")}" aria-label="Next"> <span
